...
namespace DoozyUI
{
    public class UINavigation
    {
        ...
        {
            int count = show != null ? show.Count : 0;
            if(count == 0) { return; }
        ...
        {
            int count = hide != null ? hide.Count : 0;
            if(count == 0) { return; }
            for(int i = 0; i < count; i++)
            {
        ...
        /// </summary>
        public static void UpdateTheNavigationHistory(NavigationPointerData navData, bool instantAction = false)
        {
            ...
            Show(navData.show, instantAction);
            Hide(navData.hide, instantAction);
            ...
            if(navData.addToNavigationHistory)
            {
                ...
                int count = visibleHideElementsList != null ? visibleHideElementsList.Count : 0;
                if(count > 0)
                {
                    for(int i = 0; i < count; i++)
                    {
        ...
        public static void ShowAndHide(List<NavigationPointer> show, List<NavigationPointer> hide, bool addToNavigationHistory, bool instantAction = false)
        {
            UpdateTheNavigationHistory
                (
                    new NavigationPointerData()
                    {
                        addToNavigationHistory = addToNavigationHistory,
                        show = new List<NavigationPointer>(show),
                        hide = new List<NavigationPointer>(hide)
                    },
                    instantAction
                );
        }
        ...
        {
            List<NavigationPointer> showPointers = new List<NavigationPointer> //create a NavigationPointer List and add the UIElement that needs to get shown
            {
                new NavigationPointer(elementCategory, elementName)
            };

            List<UIElement> visibleUIElements = UIElement.GetVisibleUIElements(); //get all visible UIElements

            List<NavigationPointer> hidePointers = new List<NavigationPointer>(); //create a NavigationPointer List of all the visible UIElements that will get hidden
            for(int i = 0; i < visibleUIElements.Count; i++)
            {
                hidePointers.Add(new NavigationPointer(visibleUIElements[i].elementCategory, visibleUIElements[i].elementName));
            }
            ...
            ShowAndHide
                (
                    showPointers,           //SHOW
                    hidePointers,           //HIDE
                    addToNavigationHistory, //ADD TO NAVIGATION HISTORY
        ...
        public static void ShowUiElement(string elementName, string elementCategory, bool addToNavigationHistory, bool instantAction = false)
        {
            ShowAndHide
                (
                    new List<NavigationPointer>() { new NavigationPointer(elementCategory, elementName) },
                    new List<NavigationPointer>() { },
    ...
    [UnityEditor.CustomPropertyDrawer(typeof(NavigationPointerData))]
    public class NavigationPointerDataDrawer : UnityEditor.PropertyDrawer
    {
